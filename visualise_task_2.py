...
for i in range(100,107):
    calculate_metrics(input_file= "buffer_task_"+str(i)+".txt")

figure_1 = plt.figure(1)

plt.plot([2, 2.5, 3, 3.5, 4, 4.5, 5], super_means_t)
print(super_means_t)
print(confidence_interval_t)
x = 1.5
for i in range(7):    
    plt.plot(x+0.5, confidence_interval_t[i][0], 'bo')
    plt.plot(x+0.5, confidence_interval_t[i][1], 'bo')
    x =x +0.5
interactive(True)
plt.title("Plot of the super mean of service completion time: T vs range of Buffer size")
plt.ylabel('Super Mean value for servie completion T')
plt.xlabel('Buffer size')
figure_1.savefig("plot_of_T_buffer.png")
figure_1.show()


figure_2 = plt.figure(2)
plt.plot([2, 2.5, 3, 3.5, 4, 4.5, 5], super_means_d)
#print(super_means_d)
#print(confidence_interval_d)

x=1.5
for i in range(7):
    plt.plot(x+0.5, confidence_interval_d[i][0], 'bo')
    plt.plot(x+0.5, confidence_interval_d[i][1], 'bo')
    x =x +0.5
interactive(False)
plt.title("Plot of the super mean of orbital time: D vs range of Buffer size")
plt.ylabel('Super Mean value for orbital times D')
plt.xlabel('Buffer size')
figure_2.savefig("plot_of_D_buffer.png")
figure_2.show()
input()
